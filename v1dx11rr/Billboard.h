...
public:
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam, float xx, float zz, float posy, float escala, vector2* uv1 = nullptr, vector2* uv2 = nullptr, vector2* uv3 = nullptr, vector2* uv4 = nullptr, int frame = 0)
	{
		posx = xx;
		posz = zz;

		if (uv1 != nullptr && uv2 != nullptr && uv3 != nullptr && uv4 != nullptr)
		{
			// Lógica para billboards dinámicos (tu código actual)
			vertices = new VertexComponent[4];
		...
		// Paso de datos (esto se hace siempre)
		...
		d3dContext->PSSetShaderResources(1, 1, &normalMap); // Asegúrate de que esto sea necesario para tu caso estático
